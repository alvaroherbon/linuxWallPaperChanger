...
export const LinuxwallpaperchangerWindow = GObject.registerClass({
  ...
  InternalChildren: ['search_entry', 'box_container', 'set_background_button'], // Asegúrate de incluir el id del GtkCenterBox
}, class LinuxwallpaperchangerWindow extends Adw.ApplicationWindow {
  constructor(application) {
    super({ application });
    this._search_entry.connect('activate', this._onSearchEntryActivate.bind(this));
    this._soupSession = new Soup.Session();

    const openAction = new Gio.SimpleAction({ name: 'open' });
    openAction.connect('activate', () => this._onSetBackgroundButtonClicked());
    ...
    // Inicializar la búsqueda para mostrar una imagen al iniciar
  ...
  _onSearchEntryActivate() {
    ...
    const url = `https://api.unsplash.com/photos/random?query=${encodeURIComponent(query)}&collections=wallpapers&count=1&orientation=landscape&client_id=${UNSPLASH_ACCESS_KEY}&${cacheBuster}`;

    const request = Soup.Message.new('GET', url);
    this._soupSession.send_and_read_async(request, GLib.PRIORITY_DEFAULT, null, (session, result) => {
      try {
        ...
        if (data.length > 0) {
          ...
          this._downloadAndSaveImage(imageUrl);
        } else {
  ...
  _downloadAndSaveImage(url) {
    ...
    this._soupSession.send_and_read_async(request, GLib.PRIORITY_DEFAULT, null, (session, result) => {
      try {
        ...
        const picturesDir = GLib.get_user_special_dir(GLib.UserDirectory.PICTURES);
        this._filePath = GLib.build_filenamev([picturesDir, 'unsplash_image.jpg']);
        ...
        this._createAndAddPicture(this._filePath); // Usa filePath directamente
      } catch (error) {
